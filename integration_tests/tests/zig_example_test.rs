...
fn run_zig_example(
    ...
    let zig_wasm_path = workspace_root
        .join("..")
        .join("shopify-function-zig/zig-out/bin/example_with_targets.merged.wasm");

    if !zig_wasm_path.exists() {
        anyhow::bail!(
            "Zig example WASM not found at {:?}. Build it first with: cd ../shopify-function-zig && zig build -Doptimize=ReleaseSmall",
            zig_wasm_path
        );
    }

    let module = Module::from_file(&engine, &zig_wasm_path)?;
...
fn test_zig_target_a() -> Result<()> {
    ...
    let (output, logs) = run_zig_example("target_a", input)?;

    eprintln!("target_a output: {}", serde_json::to_string_pretty(&output)?);
    eprintln!("target_a logs: {:?}", logs);
...
#[test]
fn test_zig_target_b() -> Result<()> {
    let input = serde_json::json!({
        "id": "gid://shopify/Product/123",
        "targetAResult": 200
    });

    let (output, logs) = run_zig_example("target_b", input)?;

    eprintln!("target_b output: {}", serde_json::to_string_pretty(&output)?);
    eprintln!("target_b logs: {:?}", logs);

    // Verify output matches expected Rust behavior
    assert_eq!(
        output,
        serde_json::json!({
