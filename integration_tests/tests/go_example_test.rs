...
fn run_go_example(
    ...
    let go_wasm_path = workspace_root
        .join("..")
        .join("shopify-function-go/build/example_with_targets.merged.wasm");

    if !go_wasm_path.exists() {
        anyhow::bail!(
            "Go example WASM not found at {:?}. Build and trampoline it first.",
            go_wasm_path
        );
    }

    let module = Module::from_file(&engine, &go_wasm_path)?;
    ...
    // TinyGo needs WASI stubs
    linker.func_wrap("wasi_snapshot_preview1", "fd_write", |_caller: wasmtime::Caller<'_, ()>, _fd: i32, _iovs: i32, _iovs_len: i32, _nwritten: i32| -> i32 { 0 })?;
    linker.func_wrap("wasi_snapshot_preview1", "proc_exit", |_caller: wasmtime::Caller<'_, ()>, _code: i32| {})?;
    linker.func_wrap("wasi_snapshot_preview1", "random_get", |_caller: wasmtime::Caller<'_, ()>, _buf: i32, _buf_len: i32| -> i32 { 0 })?;
    linker.func_wrap("wasi_snapshot_preview1", "args_sizes_get", |_caller: wasmtime::Caller<'_, ()>, _argc: i32, _argv_buf_size: i32| -> i32 { 0 })?;
    linker.func_wrap("wasi_snapshot_preview1", "args_get", |_caller: wasmtime::Caller<'_, ()>, _argv: i32, _argv_buf: i32| -> i32 { 0 })?;

    // Instantiate the Go module
    store.set_fuel(u64::MAX)?;
...
fn test_go_target_a() -> Result<()> {
    ...
    let (output, logs) = run_go_example("target_a", input)?;

    eprintln!("Go target_a output: {}", serde_json::to_string_pretty(&output)?);
    eprintln!("Go target_a logs: {:?}", logs);
...
#[test]
fn test_go_target_b() -> Result<()> {
    let input = serde_json::json!({
        "id": "gid://shopify/Product/123",
        "targetAResult": 200
    });

    let (output, logs) = run_go_example("target_b", input)?;

    eprintln!("Go target_b output: {}", serde_json::to_string_pretty(&output)?);
    eprintln!("Go target_b logs: {:?}", logs);
