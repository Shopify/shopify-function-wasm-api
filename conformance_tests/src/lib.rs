...
pub fn run_wasm_module(
    ...
    run_wasm_module_export(wasm_path, "_start", input_json)
}

/// Runs a named export from a pre-compiled SDK example through the
/// provider+trampoline pipeline.
pub fn run_wasm_module_export(
    wasm_path: impl AsRef<Path>,
    export_name: &str,
    input_json: &serde_json::Value,
) -> Result<(serde_json::Value, String)> {
    ...
    let source_name = wasm_path
        .file_stem()
        .and_then(|name| name.to_str())
        .unwrap_or("sdk");
    let temp_id = TEMP_FILE_ID.fetch_add(1, Ordering::Relaxed);
    let trampolined_path = std::env::temp_dir().join(format!(
        "shopify-function-{source_name}-{}-{temp_id}.trampolined.wasm",
        std::process::id()
    ));
